...
FIRST_MARKER_FOV_THRESHOLD = 0.1     #0.03 #Threshold for alignment to marker in radians
SECOND_MARKER_FOV_THRESHOLD = 0.1
TURN_CONSTANT = 0.01325         #0.0155, #0.01111, #0.0132
SMOOTH_TURN_CONSTANT = 0.023
TURN_SPEED = 1                  #0.1 / 0.3
SCAN_ANGLE = 35                 #previously 35 deg
...
MINIMUM_CAMERA_RANGE = 1000     #orig 1000, 450
...
class MyRobot(Robot):
    def __init__(self):
        ...
        super().__init__(wait_for_start = True, trace_logging=False, debug=False, ignored_arduinos=[ARDUINO_SN], raw_ports=[(ARDUINO_SN, BAUD_RATE)])
...
##            sys.exit()
        ...
        self.startRobot()
        self.rotateRobot(-60)
        self.restInterupt(0.2)
        #self.moveRobotDistanceForward(50)
...
##                        self.power_board.piezo.buzz(FS4, 0.2, blocking=True)
                ...
                self.power_board.piezo.buzz(F5, 0.4, blocking=True)
                self.power_board.piezo.buzz(C5, 0.1, blocking=True)
                self.power_board.piezo.buzz(F5, 1, blocking=True)
                self.power_board.piezo.buzz(AS4, 0.2, blocking=True)
                self.power_board.piezo.buzz(C5, 0.5, blocking=True)
                self.power_board.piezo.buzz(F4, 0.9, blocking=True)
                self.restInterupt(0.05)
                self.power_board.piezo.buzz(A4, 0.015, blocking=True)
                self.power_board.piezo.buzz(C5, 0.015, blocking=True)
                self.power_board.piezo.buzz(F5, 0.2, blocking=True)
                self.power_board.piezo.buzz(C5, 0.1, blocking=True)
                self.power_board.piezo.buzz(G5, 0.1, blocking=True)
                self.power_board.piezo.buzz(F5, 1.4, blocking=True)
...
##        self.power_board.piezo.buzz(Note.D6, 0.5)
    ...
    def armUp(self):               #move arm up until endstop
        ...
        while self.top_endstop == False:
            ...
            self.sleep(0.2)
            if time.time() - self.current_action_time > ARM_TIME_LIMIT:
                self.moveRobot(-1, 1)
                self.armDown()
                self.current_action_time = time.time()
                self.armUp()
    ...
    def armDown(self):                        #move arm down until endstop
        self.motor_boards[ARMVACBOARD].motors[1].power = -0.8
    ...
    def pickBox(self):                                     #grab a box
        self.motor_boards[ARMVACBOARD].motors[0].power = 0.8      #set vacuum on, must come before armUp
        self.armDown()
        self.motor_boards[ARMVACBOARD].motors[1].power = 0.2    #continue to add rotational pressure
        if self.marker_target not in self.visited_markers:
    ...
    def dropBox(self):                  # dump asteroid in spaceship
        ...
        self.letGoBox()                  # pray
        self.restInterupt(5)            # WAIIIIIITTTT !!!
        self.undocking = True
        self.clearCollision()
        self.startRobot()
        self.clearCollision()
    ...
    def isVisited(self, marker_id):
        if len(self.visited_markers) > 0:
            for elem in self.visited_markers:
                if elem.id == marker_id:
                    return True
            return False
        else:
            return False
    ...
    def getTarget(self, targettype):        #requires an update beforehand!!
        self.log2(("getTarget start",targettype))
        ...
        for temp_marker in self.markers: #find closed marker of desired type
            self.log2("TMPMRKR,",temp_marker.id)
            if targettype >= 0 and temp_marker.id == targettype:    #reused targettype as explicit index
                ...
                break
            elif targettype == MYSPACESHIP and (temp_marker.id == self.mySpaceShip or temp_marker.id == self.mySpaceShip+5):
                result = True
                ...
                self.log2("temp_markerID=",temp_marker.id)
                if not self.isVisited(temp_marker.id):
                ...
                break
            elif targettype == BOUNDARY and temp_marker.id >= (self.zone*7) and temp_marker.id < (self.zone*7)+7:
                result = True
                self.marker_target = temp_marker
                break
            else:
                self.log2(("Target ", targettype, " not equal to temp_marker.id ", temp_marker.id))
        
        if result == True:
            self.power_board.piezo.buzz(Note.B6, 0.1)
            self.power_board.piezo.buzz(Note.B6, 0.1)
            self.power_board.piezo.buzz(Note.B6, 0.1)
            self.rotated_scan_angle = 0
            self.startRobot()
            self.log2(("Found target from camera ",temp_marker.id))
            try:
                self.ground_distance = math.sqrt((self.marker_target.position.distance**2) - (GROUND_CAMERA_HEIGHT**2))
            except AttributeError:
                self.log2(("getTarget requires an update(True) call beforehand"))
        else:
            self.log2(("NOT found from camera where TARGETTYPE Value = ",targettype))
    ...
    def updateMarkers(self):
        ...
        try:
            ...
            #self.camera.save(self.usbkey / "initial-view.jpg")
        except RuntimeError:
            for i in range(0, 5):
                self.log2("Camera not turned on or plugged in!! Panic!")
                self.power_board.piezo.buzz(Note.C6, 0.2, blocking=True)
    ...
    def update(self,photo):
        ...
        if self.stop_motors == True and photo and self.camera_activated:
            self.updateMarkers()
...
##                self.log(("Updated Sensors, Camera Inactive, Measure F,B", self.front_sensor_distance, self.back_sensor_distance))
    ...
    def moveRobot(self, speed, duration):   #Move at specified speed and for duration
        if not self.stop_motors:
            ...
            while now_time < endtime and not self.frontCollision():
            ...
            self.log2("moveRobot failed; stop_motors is True")
...
##            self.stopRobot()
    ...
    def moveTowardsObject(self, Object, decimal, box_offset):
...
##            box_offset = 100
    ...
    def calculateAngleFromArmOffset(self):
        # code to try and align for arm
        a = (self.ground_distance*math.sin(math.radians(90)-self.marker_target.position.horizontal_angle)) / (math.sin(math.radians(90)))
        self.log2("A", a)
        if self.marker_target.position.horizontal_angle > 0:
            self.log2("Angle > 0")
            c = abs(math.sqrt( math.pow(self.ground_distance, 2) - math.pow(a, 2) )+ARM_CAMERA_OFFSET_MM)
            F = math.atan(a/(c))
            theta = self.marker_target.position.horizontal_angle + ( (math.radians(90) - self.marker_target.position.horizontal_angle - F) )
            
        elif self.marker_target.position.horizontal_angle < 0:
            self.log2("Angle < 0")
            c = abs(math.sqrt( math.pow(self.ground_distance, 2) - math.pow(a, 2) )-ARM_CAMERA_OFFSET_MM)
            F = math.atan(a/(c))
            theta = math.radians(90) - F
            theta = -theta
        else:
            theta = self.marker_target.position.horizontal_angle
        self.log2("C", c)
        self.log2("F", F)
        self.log2("HORIZ_ANG", math.degrees(self.marker_target.position.horizontal_angle))
        self.log2("theta", math.degrees(theta))
    ...
    def findObject(self, Object):
        ...
        self.update(True)
        if self.getTarget(Object):
            self.log2("GET TARGET Object == ", Object)
            while not self.frontCollision() and not self.arrived:
                if self.correctAngle(Object, math.radians(SECOND_MARKER_FOV_THRESHOLD)):
                    self.log2("First correctAngle")
                    if Object == BOUNDARY or self.isBoundary(Object):
                        self.log2("FOUND Boundary!", Object)
                        self.startRobot()
                        self.moveTowardsObject(Object, 0.7, 0) # was Object, 1, 30
                        self.arrived = True
                        self.scan_left = True
                    elif Object == MYSPACESHIP or Object == self.mySpaceShip or Object == self.mySpaceShip+5:
                        self.log2("FOUND Myspaceship!", Object)
                        self.log2("Move to spaceship, marker:", self.marker_target.id)
##                                self.wait_start()
                        self.startRobot()
                        #self.rotateRobot(5)
                        self.startRobot()
                        self.armUp()
                        self.moveTowardsObject(Object, 1, -150)
                        self.arrived = True
                        self.scan_left = False
##                    elif self.ground_distance < MINIMUM_CAMERA_RANGE:
##                        self.log2("ground_distance < MINIMUM_CAMERA_RANGE")
##                        self.moveTowardsObject(Object, 1, 100) # was Object, 1, 30
##                        self.arrived = True
                    elif Object == ASTEROID:
                        self.log2("FOUND ASTEROID! Object:", Object)
                        self.startRobot()
                        self.moveTowardsObject(Object, 1, 300)
                        self.arrived = True
                        self.scan_left = False
                    else:
                    ...
                    return self.findObject(Object)
                if self.frontCollision() and self.arrived:
                    self.log2(("findObject(", Object, ") aborted; frontCollision is True, arrived at box"))
                    self.scan_left = False
                    self.rotated_scan_angle = 0
                    return True
                elif self.frontCollision() and not self.arrived:
                    self.log2("findObject(", Object, ") aborted; frontCollision is True, arrived is False")
                    return self.findObject(Object)
        else:
            self.log2("RESCAN")
            self.arrived = False
            self.startRobot()
            if self.scan_left:
                self.rotateRobot(SCAN_ANGLE*-1)
            else:
                self.rotateRobot(SCAN_ANGLE)
            self.rotated_scan_angle += SCAN_ANGLE
            self.log2("VISITED MARKERS", self.visited_markers)
            if self.rotated_scan_angle > 360:
                self.startRobot()
                self.rotateRobot(-90)
                self.moveRobotDistanceForward(250)
                self.rotated_scan_angle = 0
            self.stopRobot()
            self.update(True)
            return self.findObject(Object)
...
while not done:
    ...
    robot.log2("ROBOT.ZONE: ", robot.zone)
    robot.findObject((robot.zone*7)+3)                   #We've decided to look for ID 2. Its a border marker near out spaceship.
    robot.clearCollision()
...
##    robot.log2("ARMUP")
##    robot.armUp()
##    robot.moveRobotDistanceForward(100)
##    robot.restInterupt(1)
##    robot.log2("letGoBox")
##    robot.letGoBox()
##    robot.wait_start()
