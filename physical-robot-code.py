...
class MyRobot(Robot):
    ...
    #defunct Arm Functions
    def armInit(self):
        self.armDown()
        self.armUp()
        self.power_board.piezo.buzz(Note.B6, 0.5, blocking=True)#do a beep :)
        self.power_board.piezo.buzz(Note.D6, 0.5)
        
    def moveArm1(self, decimal):             #move the arm up/down on % 
    ...
    def moveArmUp(self, decimal):           #move arm a percentage up assuming start pos is at bottom
        self.motor_boards["SR0XK1E"].motors[1].power = 0.2
        self.restInterupt(decimal * ARM_MOVE_TIME)
        
    def moveArmDown(self, decimal):         #move arm a percentage down assuming start pos is at top
        self.motor_boards["SR0XK1E"].motors[1].power = -0.2
        self.restInterupt(decimal * ARM_MOVE_TIME)

        
    def armDown(self):
        self.motor_boards["SR0XK1E"].motors[1].power = -0.2     #tells arm to move down
        self.top_endstop = robot.arduino.command("T")           #queries for T_ENDSTOP Val
        while self.top_endstop != "T:1\r":                      #until endstop is hit,
            self.top_endstop = robot.arduino.command("T")       #query again.
            self.sleep(0.1)
        self.motor_boards["SR0XK1E"].motors[1].power = 0        #stop arm! endstop hit
        self.power_board.piezo.buzz(Note.C6, 0.5, blocking=True)#do a beep :)
        self.power_board.piezo.buzz(Note.D6, 0.5)
        self.log("ARM AT BOTTOM")

    def armUp(self):
        self.motor_boards["SR0XK1E"].motors[1].power = 0.2      #tells arm to move up
        self.bottom_endstop = robot.arduino.command("B")        #queries for B_ENDSTOP
        while self.bottom_endstop != "B:1\r":                   #until endstop hit,
            self.bottom_endstop = robot.arduino.command("B")    #query again.
            self.sleep(0.1)
        self.motor_boards["SR0XK1E"].motors[1].power = 0
        self.power_board.piezo.buzz(Note.D6, 0.5, blocking=True)#buzz :D
        self.power_board.piezo.buzz(Note.C6, 0.5)
        self.log("ARM AT TOP")
        
    def gripperOpen(self):
        self.motor_boards["SR0XK1E"].motors[0].power = 1        #lower the box before turning off vacuum
        self.armDown()
        self.motor_boards["SR0XK1E"].motors[0].power = 0        #let go
        self.armUp()

    def gripperClose(self):                                     #grab a box
        self.motor_boards["SR0XK1E"].motors[0].power = 1        #set vacuum on, must come before armDown
        self.armDown()                                          #move arm down
        self.armUp()

    # defunct simulator code
    def gripperMove(self,value):            #worker function for gripper
        ...
        self.restInterupt(0.5)
        self.servo_board.servos[1].position = None
    def gripperOpen1(self):                  # open gripper fully
        self.gripperMove(-1)
    ...
    def gripperClose1(self):                 # close gripper fully
        self.gripperMove(1)
    ...
    def update(self):
        ...
        if self.front_collision or self.back_collision or self.left_collision or self.right_collision or self.gripper_collision_left or self.gripper_collision_right : 
            ...
            self.stopRobot()                   
        if self.motor_boards["SR0UF7"].motors[0].power == 0 and self.motor_boards["SR0UF7"].motors[1].power == 0:
            self.restInterupt(0.1)
            self.updateMarkers()
    ...
    def delay(self, angle):
        ...
        if time > 0:
            ...
            self.motor_boards["SR0UF7"].motors[0].power = 0
            self.motor_boards["SR0UF7"].motors[1].power = 0
    ...
    def rotateRobot(self, angle, delay):
        ...
        if angle < 0:                       #left
            ...
            self.motor_boards["SR0UF7"].motors[0].power = -0.1
            self.motor_boards["SR0UF7"].motors[1].power = 0.1
        elif angle > 0:                     #right
    ...
    def moveRobot(self, speed, duration):   #Move at specified speed and for duration
        if not self.stop_motors:
            ...
            while now_time < endtime and not self.stop_motors:
                self.update()
                self.motor_boards["SR0UF7"].motors[0].power = speed
                self.motor_boards["SR0UF7"].motors[1].power = speed
